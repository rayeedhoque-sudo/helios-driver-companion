...
export class NT4_Client {
  ...
  private async connectOnAlive() {
    ...
    // ponytail: LOCAL MODIFICATION. Upstream probes ports with an HTTP `fetch` before
    // opening the socket, but that fetch is CORS-blocked from this app's file:// renderer
    // (main.ts uses loadFile + default webSecurity) and would retry forever. WebSocket
    // handshakes are exempt from CORS, so connect the WS directly on the first configured
    // port, retried every ~1s (this is also the reconnect backoff).
    this.activeServerPort = this.serverPorts[0];
    setTimeout(() => {
      if (this.serverConnectionRequested && this.ws === null) this.ws_connect();
    }, 1000);
  }
  ...
  disconnect() {
    if (this.serverConnectionRequested) {
      ...
      if (this.serverConnectionActive && this.ws) {
        ...
        // ponytail: LOCAL FIX. A socket still CONNECTING (not yet open) must be torn
        // down too — otherwise it is leaked, opens later against this dead client, and
        // its ws_onOpen/data fight the replacement client over the shared callbacks
        // (seen as old-host values after a host/sim switch mid-connect).
        this.ws?.close();
        this.rttWs?.close();
        this.ws = null;
        this.rttWs = null;
  ...
  private ws_sendBinary(data: Uint8Array, rttWs = false) {
    ...
    if (ws && ws.readyState === WebSocket.OPEN) {
      // ponytail: cast for TS7 lib.dom — msgpack returns Uint8Array<ArrayBufferLike>, send() wants BufferSource.
  ...
  private ws_onMessage(event: MessageEvent, rttOnly: boolean, source: WebSocket) {
    // ponytail: LOCAL FIX (mirrors ws_onClose/ws_onError): drop frames already queued
    // on a socket that is no longer current, so a stale socket can't re-arm the
    // disconnect timeout or deliver old-host data after a switch.
    if (source !== (rttOnly ? this.rttWs : this.ws)) return;
    ...
    if (typeof event.data === "string") {
      ...
      for (let unpackedData of this.msgpackDecoder.decodeMulti(event.data)) {
        ...
        // Validate types. ponytail: LOCAL FIX — `continue`, not `return`: one malformed
        // frame must not discard the valid frames packed after it in the same message.
        ...
        if (topicID >= 0) {
          ...
          if (!topic) {
            ...
            continue; // ponytail: LOCAL FIX — see validation block above
          }
  ...
  private ws_connect(rttWs = false) {
    ...
    ws.addEventListener("message", (event: MessageEvent) => this.ws_onMessage(event, rttWs, ws));
    if (!rttWs) {
